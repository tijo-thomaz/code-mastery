...
function useLocalStore(ket, init) {
  const [value, setValue] = useState;
  ...
  return (
    <div>
      <input
        ...
        placeholder="Persists on refresh!"
      />
      <p>Stored: {name || "—"}</p>
    </div>
  );
...
export function R24_DebounceSearch() {
  ...
  const results = debouncedQuery
    ? allItems.filter((r) =>
        r.toLowerCase().includes(debouncedQuery.toLowerCase()),
      )
    : [];
...
const fileTreeData = [
  {
    ...
    children: [
      {
        name: "components",
        children: [{ name: "App.jsx" }, { name: "Header.jsx" }],
      },
...
function NestedBox({ colors }) {
  ...
  return (
    ...
    >
      <span
        style={{ fontSize: 12, color: "white", textShadow: "0 0 2px black" }}
      >
        {first}
      </span>
...
// 🔥 R28 Portal Version — The CORRECT interview answer
// ───────────────────────────────────────────────────────────────
// WHY Portal? Renders outside parent DOM tree → avoids:
//   1. overflow:hidden clipping
//   2. z-index stacking context issues
//   3. CSS transform breaking position:fixed
// React events still bubble through the component tree (not DOM tree).
// ───────────────────────────────────────────────────────────────
...
function PortalModal({ isOpen, onClose, title, children }) {
  ...
  return createPortal(
    ...
    document.body, // ← renders at <body>, escaping any parent overflow/z-index
  );
}

export function R28_PortalModalDemo() {
  const [isOpen, setIsOpen] = useState(false);
  return (
    // Even inside overflow:hidden — Portal modal works fine!
    <div
      style={{
        overflow: "hidden",
        height: 100,
    ...
    >
      <p>
        This box has overflow:hidden. Without Portal, the modal would be
        clipped.
      </p>
      ...
      <PortalModal
        ...
        title="Portal Modal ✨"
      >
        <p>I render at document.body via createPortal!</p>
        <p>Press ESC or click backdrop to close.</p>
      </PortalModal>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 400 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
        ...
        {step === 3 && (
          <button onClick={() => alert(JSON.stringify(form, null, 2))}>
            Submit
          </button>
        )}
      </div>
